...
.refine(
    (data) => data.player1Score !== data.player2Score,
    {
...
.refine(
    (data) => Math.max(data.player1Score, data.player2Score) >= 11,
    {
...
.refine(
    (data) => {
        ...
        // If max score is exactly 11, min score must be 9 or less
        // If max score is more than 11, the difference must be at least 2
        if (maxScore === 11) {
            return minScore <= 9
        }
    ...
    {
        message: 'Spilleren må vinne med minst 2 poengs margin hvis resultatet er over 11-9',
        path: ['player1Score'],
    }
)
...
export const playerNameSchema = z
    ...
    .regex(/^[a-zA-ZæøåÆØÅ0-9\s\-_.]+$/, 'Spillernavn kan kun inneholde bokstaver, tall, mellomrom og grunnleggende tegn')

/**
 * Validation schema for new match form
 */
